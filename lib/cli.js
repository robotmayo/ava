...
exports.run = () => { // eslint-disable-line complexity
	...
	const cli = meow(`
		...
		Options
		  --watch, -w             Re-run tests when tests and source files change
			...
			--hook, -m              A file to hook function to be run whenever the run state changes
	...
	if(conf.hook) {
		try {
			...
			const m = resolveCwd.silent(conf.hook);
			if(m == null) throw new Error("Unable to find custom hook");
			const Hook = require(m);
			hook = new Hook();
		}catch(error) {
			exit(error.message);
		}
	...
	api.on('run', plan => {
		...
		if(hook) {
			hook.startRun();
			plan.status.on('stateChange', evt => hook.onStateChange(evt));
		}
	});
