...
exports.run = () => { // eslint-disable-line complexity
	...
	if(conf.reporter) {
		try {
			...
			const m = resolveCwd.silent(conf.reporter);
			if(m == null) throw new Error("Unable to find custom reporter");
			CustomReporter = require(m);
		}catch(error) {
			exit(error.message);
		}
	}
